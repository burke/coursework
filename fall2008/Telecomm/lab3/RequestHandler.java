...
/* Constructors */
		 ...
		 public RequestHandler(Socket incoming, int num_vis_i)throws IOException {
					numVisited = num_vis_i;
					client = incoming;
							 ...
							 try {
										...
										if (theFile.exists()) {
												 ...
												 if(!lastModified.after(ifModifiedSince)) {
															String response = getResponseHeader(HTTP_NOT_MODIFIED,fileName,numVisited);
															outToClient.println(response);
															outToClient.flush();
												 } else {
															String response = getResponseHeader(HTTP_OK,fileName,numVisited);
															outToClient.println(response);
															outToClient.flush();
...
//   After sending the header, send the requested file to the client.
												 ...
												 String response = getResponseHeader(HTTP_NOT_FOUND,"",numVisited);
												 outToClient.println(response);
												 outToClient.flush();
										}
							 }
...
// be returned to the client (in addition to any data that might be returned).
		 public String getResponseHeader(int statusCode, String fileName) {
					return getResponseHeader(statusCode, fileName, 0);}
		 public String getResponseHeader(int statusCode, String fileName, int numVisited) {
					...
					buffer.append("Num-of-visited: " + numVisited + "\n");
